...
# check-debug-usage.sh - Warns about console.log usage instead of debug utilities
...
# Skip test files, node_modules, and debug utility files themselves
if [[ "$relative_path" == *"test"* ]] || \
   [[ "$relative_path" == *"node_modules"* ]] || \
   [[ "$relative_path" == *"lib/debug.ts" ]] || \
   [[ "$relative_path" == *".test."* ]] || \
...
if grep -qE "console\.(log|error|warn|debug)" "$relative_path" 2>/dev/null; then
  # Check if file already imports debug utilities
  if ! grep -qE "from ['\"]@/lib/debug['\"]" "$relative_path" 2>/dev/null; then
    log "⚠️  Consider using debug utilities from @/lib/debug instead of console.log in: $relative_path"
  fi
fi
