...
# type-check.sh - Hook script that runs TypeScript type checking after TS/TSX file edits
...
case "$relative_path" in
  *.ts|*.tsx)
    log "Type checking: $relative_path"
    bunx tsc --noEmit 2>&1 | tee -a "$LOG_FILE" >&2 || true
    ;;
  *)
...
# Exit successfully (don't block agent operations even if type checking has errors)
exit 0
