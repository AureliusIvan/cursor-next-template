...
export function GET() {
  ...
  const state = {
    ...
    handleCompanyEvent: null as ((company: unknown) => void) | null,
    isClosed: false,
  ...
  const cleanup = () => {
    ...
    if (state.handleCompanyEvent) {
      companyEvents.off("company.created", state.handleCompanyEvent);
      state.handleCompanyEvent = null;
    }
  };
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      companyEvents.on("company.created", state.handleCompanyEvent);
